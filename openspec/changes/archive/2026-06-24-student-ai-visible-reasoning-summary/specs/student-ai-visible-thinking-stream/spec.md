...
When model reasoning summaries are unavailable, the student assistant SHALL expose truthful high-level agent progress rather than preset fake thinking.
...
- **THEN** it MAY emit `thinking(source="agent_trace", phase="policy", message="正在判断问题范围")`
...
- **THEN** it MAY emit `thinking(source="agent_trace", phase="retrieval", message="正在整理课程上下文")`
...
- **THEN** it MAY emit `thinking(source="agent_trace", phase="retrieval", message="正在检索课程资料")`
- **AND** it MUST NOT claim retrieval ran when the classification or feature switch skipped retrieval.

#### Scenario: Retrieval is skipped or has no match
...
- **THEN** the backend MAY emit a high-level trace message such as `正在根据课程知识组织回答`
...
- **THEN** it MAY emit `thinking(source="agent_trace", phase="generation", message="正在组织回答")`
...
- **THEN** the backend MAY emit `thinking(source="agent_trace", phase="fallback", message="正在使用本地兜底生成回答")`
