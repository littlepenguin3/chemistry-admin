...
- Keep catalog search as a compact browser-header action that opens the unified search page instead of a pinned bottom search field.
- Render the finite catalog footer inside the row list, summarizing non-zero current-layer directory and experiment counts and aligning to the row-list bottom for short lists.
...
- Update mobile visual QA and tests to lock the expanded/collapsed header behavior, catalog discoverability, directory-shell continuity, and footer geometry.
...
- Affects student H5 styles in learning/catalog/mobile CSS and the catalog-footer QA script.
