...
### 4. Explain wrong answers from the AI report first, with local fallback only on failure

Question explanations should be credible even when the question bank lacks explanation text. Use this precedence:

1. Persisted report `mistake_explanation` generated by the current AI assessment-report chain for this completed attempt.
2. Stored question `explanation` from the current question bank or report payload when AI is unavailable or unsafe.
3. Local fallback template:
   - "本题应先识别题干中的实验现象/试剂/条件，再对照参考答案判断。你的作答为 X，参考答案为 Y，建议回看相关实验点位。"

When `mistake_explanation.source` is `ai`, the old UI may label it `AI错题解析` and does not need prominent `你的答案` / `参考答案` comparison boxes. If the source is fallback, the visible copy should still be useful and not mention internal failure.
