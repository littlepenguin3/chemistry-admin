...
- Show wrong-question review in each report detail, including question stem, compact wrong/correct answer rows, and per-question `AI 解析` content; the report-level learning summary remains generated by the real backend AI path, while per-question explanation may use stored question-bank/database explanation until structured per-question AI output exists.
