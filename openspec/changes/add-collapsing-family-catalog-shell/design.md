...
The family page should read as two vertical regions: an upper element-context region and a lower `章节学习目录` sheet. The sheet owns catalog title, directory rows, and a sticky bottom search field for the current catalog layer.

Rationale: the user mental model is "element context above, learning directory below." A sheet boundary makes the catalog feel like its own browser workspace instead of another card below the element card. The bottom search mirrors modern content apps where search belongs to the lower work area.
...
4. Update styles and tests for expanded/collapsed header and catalog visibility.
5. Run student typecheck/e2e and OpenSpec validation.
