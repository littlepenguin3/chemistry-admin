...
def _load_visible_feedback(session: Any, feedback_id: str, user: Any) -> dict[str, Any]:
    visibility_sql, params = feedback_visibility_sql(user, "sf")
...
def feedback_summary(user: Any) -> FeedbackSummaryResponse:
    visibility_sql, params = feedback_visibility_sql(user, "sf")
...
def get_feedback(feedback_id: str, user: Any) -> dict[str, Any]:
    with db_session() as session:
        return _load_visible_feedback(session, feedback_id, user)


def update_feedback(payload: FeedbackUpdateRequest, feedback_id: str, user: Any) -> dict[str, Any]:
...
def get_feedback_attachment(feedback_id: str, attachment_id: str, user: Any) -> dict[str, Any]:
    with db_session() as session:
        _load_visible_feedback(session, feedback_id, user)
