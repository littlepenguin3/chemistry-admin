...
def list_classes(user: AuthUser) -> list[ClassResponse]:
    if user.role == "admin":
        sql = """
...
def update_registration_settings(payload: RegistrationSettingsUpdateRequest, user: AuthUser) -> RegistrationSettingsResponse:
...
def get_class_registration_settings(class_id: str, user: AuthUser) -> RegistrationSettingsResponse:
...
def create_class(payload: ClassCreateRequest, user: AuthUser) -> ClassResponse:
...
def get_class(class_id: str, user: AuthUser) -> ClassResponse:
    require_class_access(class_id, user)
    with db_session() as session:
        row = (
            session.execute(
...
def update_class(payload: ClassUpdateRequest, class_id: str, user: AuthUser) -> ClassResponse:
    require_class_access(class_id, user)
    with db_session() as session:
        row = (
            session.execute(
...
def preview_roster_import(class_id: str, filename: str | None, content: bytes, user: AuthUser) -> dict[str, Any]:
    require_class_access(class_id, user)
...
def list_roster_students(class_id: str, user: AuthUser) -> list[RosterStudentResponse]:
    require_class_access(class_id, user)
    with db_session() as session:
        rows = (
            session.execute(
...
def disable_roster_student(class_id: str, student_id: str, user: AuthUser) -> RosterStudentResponse:
    require_class_access(class_id, user)
    normalized_student_id = _normalize_student_id(student_id)
    with db_session() as session:
        row = (
